...
class _LoginScreenState extends State<LoginScreen>
  ...
  void initState() {
    ...
    _tabFadeOut = Tween<double>(begin: 1, end: 0).animate(CurvedAnimation(
        parent: _tabCtrl,
        curve: const Interval(0.0, 0.45, curve: Curves.easeIn)));
    _tabFadeIn = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(
        parent: _tabCtrl,
        curve: const Interval(0.45, 1.0, curve: Curves.easeOut)));
    _tabScaleOut = Tween<double>(begin: 1.0, end: 0.97).animate(CurvedAnimation(
        parent: _tabCtrl,
        curve: const Interval(0.0, 0.45, curve: Curves.easeIn)));
    _tabScaleIn = Tween<double>(begin: 0.97, end: 1.0).animate(CurvedAnimation(
        parent: _tabCtrl,
  ...
  Future<void> _signInWithEmail() async {
    ...
    try {
      await FirebaseAuth.instance.signInWithEmailAndPassword(
          email: _siEmailCtrl.text.trim(), password: _siPassCtrl.text);
    } on FirebaseAuthException catch (e) {
      _setError(switch (e.code) {
  ...
  Future<void> _registerWithEmail() async {
    ...
    try {
      final cred = await FirebaseAuth.instance.createUserWithEmailAndPassword(
          email: _suEmailCtrl.text.trim(), password: _suPassCtrl.text);
                            ...
                            child: ConstrainedBox(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    SizedBox(height: screenW < 600 ? 14 : 20),
                                    _buildTagline(),
                                    const SizedBox(height: 14),
                                    _buildLangSelector(),
                                    const SizedBox(height: 14),
                                    _buildCard(),
                                    SizedBox(height: screenW < 600 ? 16 : 20),
                                  ],
                                ),
                              ),
                            ),
  ...
  Widget _buildTagline() {
    return Column(children: [
      ...
      Text(_s.taglineSub,
          textAlign: TextAlign.center,
          style:
              const TextStyle(color: _taglineSub, fontSize: 12, height: 1.5)),
    ]);
  }
  ...
  Widget _buildCard() {
    return Container(
      decoration: BoxDecoration(
        ...
        border:
            Border.all(color: _borderCard.withValues(alpha: 0.35), width: 1.5),
      ),
  ...
  Widget _formContent(int tab) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        // Form title & subtitle
        Text(tab == 0 ? _s.siTitle : _s.suTitle,
            style: const TextStyle(
                color: Color(0xFFF1FAF1),
                fontSize: 16,
                fontWeight: FontWeight.w800)),
        const SizedBox(height: 3),
        Text(tab == 0 ? _s.siSub : _s.suSub,
            style: TextStyle(
    ...
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        _field(
            ...
            obscure: _siObscure,
            suffix: _eyeBtn(
                _siObscure, () => setState(() => _siObscure = !_siObscure)),
            validator: (v) =>
        ...
        Align(
            ...
            child: TextButton(
                ...
                child: Text(_s.forgotPassword,
                    style:
                        const TextStyle(color: _accentLight, fontSize: 11)))),
    ...
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        _field(
            ...
            validator: (v) =>
                (v?.trim().isEmpty ?? true) ? _s.enterName : null),
        const SizedBox(height: 9),
        _field(
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _field(
              ...
              suffix: _eyeBtn(_suObscureConfirm,
  ...
  Widget _field({
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        labelText: label,
        labelStyle:
            TextStyle(color: _textHint.withValues(alpha: 0.6), fontSize: 12),
        prefixIcon:
            Icon(icon, color: _accentLight.withValues(alpha: 0.6), size: 18),
        suffixIcon: suffix,
        filled: true,
        fillColor: _bgField,
        isDense: true,
        ...
        child: _isLoading
            ? const SizedBox(
                ...
                child: CircularProgressIndicator(
                    strokeWidth: 2, color: _textOnGreen))
            : Text(label),
      ),
    );
  }
  ...
  Widget _divider(String label) {
    return Row(children: [
      Expanded(
          child:
              Divider(color: _accentLight.withValues(alpha: 0.2), height: 1)),
      Padding(
          padding: const EdgeInsets.symmetric(horizontal: 12),
          child: Text(label,
              style: TextStyle(
                  color: _textMuted.withValues(alpha: 0.45), fontSize: 10))),
      Expanded(
          child:
              Divider(color: _accentLight.withValues(alpha: 0.2), height: 1)),
    ]);
    ...
    return Padding(
      ...
      child: Column(children: [
        Divider(
            ...
            endIndent: 24),
        const SizedBox(height: 10),
        Text(_s.projectTagline,
            textAlign: TextAlign.center,
            style: const TextStyle(
                color: _footerPrimary, fontSize: 11, height: 1.5)),
        const SizedBox(height: 2),
        Text(_s.developedBy,
            textAlign: TextAlign.center,
            style: const TextStyle(
                color: _footerSecondary, fontSize: 10, height: 1.5)),
        const SizedBox(height: 4),
      ]),
    );
  }
      ...
      child: AnimatedContainer(
        ...
        child: Text(label,
            style: TextStyle(
              ...
              color:
                  selected ? const Color(0xFF2E7D32) : const Color(0xFF6B7A52),
            )),
      ),
    );
  }
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
                color: selected ? _accentLight : Colors.transparent, width: 2),
          ),
          child: Text(label,
              textAlign: TextAlign.center,
              style: TextStyle(
