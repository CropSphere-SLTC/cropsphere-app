...
def test_valid_prediction_input():
    data = PredictionInput(
        crop="Carrot", rainfall=500.0, temperature=25.0, humidity=80.0
    )
...
def test_invalid_crop_name():
    with pytest.raises(ValidationError) as exc_info:
        PredictionInput(
            crop="InvalidCrop", rainfall=500.0, temperature=25.0, humidity=80.0
        )
...
def test_invalid_month():
    with pytest.raises(ValidationError):
        PriceInput(crop="Carrot", month=13)  # Invalid — no 13th month
